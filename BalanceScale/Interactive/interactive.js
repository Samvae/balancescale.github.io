...
function setInteractiveParameters() {
  ...
  if (canvasInUse) {
    svgStage = new MovieClip(
      document.getElementById("svgStage").cloneNode(true)
    );
...
function animationHandler() {
  if (
    this.animationCounter > 0 &&
    this.animationCounter <= this.animationEndIndex
  ) {
    this.frames[this.frameSet[this.animationCounter - 1]].setAttribute(
      "display",
      "none"
    );
  }
  if (this.animationCounter <= this.animationEndIndex) {
    this.frames[this.frameSet[this.animationCounter]].setAttribute(
      "display",
      "inline"
    );
...
function hitTestPointBoundingBox(pass1, pass2, pass3) {
  ...
  if (Math.abs(pass1 - (pass3.x + pass3.hitX * pass3.scale)) > distanceX) {
    ...
    Math.abs(pass2 - (pass3.y + pass3.hitY * pass3.scale)) > distanceY
  ) {
    return false;
  }
  return true;
...
function hitTestBoundingBox(pass1, pass2) {
  ...
  if (
    Math.abs(
      pass1.x + pass1.hitX * pass1.scale - (pass2.x + pass2.hitX * pass2.scale)
    ) > distanceX
  ) {
    return false;
  } else if (
    Math.abs(
      pass1.y + pass1.hitY * pass1.scale - (pass2.y + pass2.hitY * pass2.scale)
    ) > distanceY
  ) {
    return false;
  }
